...
ループに入ったら、レビューが指摘を返すと実装工程がそれを反映し、再度レビューへ渡す。**往復の上限は上記「ループの上限値」の値**とし、超えたら下の停止条件に該当してオーケストレーターは握り潰さず**人間へ上げる**（ADR 0010 §D）。

反復の経緯は `progress.md`（作業記憶）に書き、恒久的な証跡は **PR にコメントで要約して残す**。コメントは冗長にせず**フォーマット化する**（往復回数 / 各回の指摘要点 / 未解決点）。PR に残すのは*決定した仕様*ではなく*経緯・証跡*であり、ADR 0004 とは衝突しない（`spec-link` と同じプロセス側の性質、ADR 0010 §C）。
