...
サブエージェント構成（ADR 0010 §D）では、オーケストレーターが実装工程とレビュー工程を往復させる。この往復は**初回 Green を PR にしてから**始まる。

**PR 作成後、往復ループを開始する前に、オーケストレーターは人間の承認（「承認する／承認しない」）を必ず取る。** 承認されるまで往復を開始しない。PR は人間が開始を判断するための成果物であり、往復の経緯を残す先でもある。
