...
**5-h / 5-i は Red を先に踏めていない** —— **既存実装が既に3属性を満たしていたため、テストは書いた時点で緑だった**。**その帰結（この2行の緑が持たない担保）は限界 10-19 が持ち、本表の期待を緩めるものではない**。**本表の対象は 5-a〜5-g（5-11 の反転元）とは別で、Cookie に付く属性である。** 見るのは **`HttpOnly` / `Secure` / `SameSite` の3属性がいずれも欠けていないこと**までであり、**`SameSite` にどの値を採るかは実装側が持つため検査しない**（5-8 (i)・AC-1-3 と同型）。**一時 Cookie（5-10 (ii)）についての同じ検査は 12-k が持つ。ここへ書き写さない**（ADR 0004。**同じ要求を両所へ置かない**）。
...
**7-c は Red を先に踏んだが、7-a / 7-b / 7-d は踏めていない** —— 表を作った時点の実装が既に「本来の処理を実行しない」「5xx を返す」を満たしており、**落ちたのは「応答の本文が空であること」（7-c）の 6 ケースだけだった**（3経路 × 2変数。Red の実測は PASS 110 / FAIL 6）。**その後に広げた網羅も Red を踏んでいない** —— **経路ごとに実際に読む5変数 ×{未設定, 空}の直積へ拡張した 24 ケースは、既存実装が既に満たすため書いた時点で緑だった**。**その帰結（7-a / 7-b / 7-d と、広げた網羅の緑が持たない担保）は限界 10-20 が持ち、本表の期待を緩めるものではない**。**受理側と対で検査する** —— 失敗側だけを検査すると、**常に失敗を返す実装が緑になる**（AC-8-5 と同じ理由。7-d と対にする）。**拒否の文言は検査しない**（AC-2 前文）。
...
| 10-19 | **検査 5-h / 5-i（セッション Cookie とロール Cookie の属性）は Red を先に踏めていない。** **既存実装が既に3属性（`HttpOnly` / `Secure` / `SameSite`）を満たしていたため、テストは書いた時点で緑であり、実装より先に落ちる状態を作れなかった**（`docs/rules/development-process.md` TDD が求める Red を、この2行については通っていない）。**帰結** —— **この2行の緑は「実装より先に落ちることを確認した」という担保を持たない**。緑が示すのは、**現在の実装が送出する `Set-Cookie` に3属性が揃っていること**までである。**本行は限界の記録であって、要求の緩和ではない** —— **本行を根拠に 5-8 (i)・5-5 (vi)・12-k・AC-8-2 のいずれも緩めない**。**5-h / 5-i の期待（3属性をいずれも欠かないこと・属性を欠いた形が緑にならないこと・応答が実際に送出する `Set-Cookie` を直接見ること）はそのまま維持する**。**AC-8-2 の本文（`web-app-scaffold.md` AC-3-7 の例外を本 Issue に適用しない）も、本行を根拠に読み替えない**。**あわせて本行は「限界として記録する」ところまでであり、実装・テストの作り直しを要求しない**（**この2行について Red を踏み直す形が要ると判断したら、変える前に停止して人間へ上げる**）。**収束先（宣言を現況へ改めること）は人間が決定した**（2026-09-04。**採らなかった案 —— AC-8-2 の例外を本 Issue へ広げる／5-h・5-i を取り下げる**。記録を消さない） |
| 10-20 | **検査 7-a / 7-b / 7-d（構成の欠落に対する応答の形）と、その後に広げた網羅は Red を先に踏めていない。** **表を作った時点の実装が既に「本来の処理を実行しない」「5xx を返す」を満たしていたため、これらの行は書いた時点で緑であり、実装より先に落ちる状態を作れなかった**（`docs/rules/development-process.md` TDD が求める Red を、これらについては通っていない）。**7-c（応答の本文が空であること）は本行の対象外である** —— **7-c は実装より先に落ちており（Red の実測 PASS 110 / FAIL 6 の 6 件はすべて 7-c）、Red を踏んでいる**。**その後に広げた網羅（経路ごとに実際に読む5変数 ×{未設定, 空}の直積へ拡張した 24 ケース）も、既存実装が既に満たすため書いた時点で緑だった**。**帰結** —— **これらの緑は「実装より先に落ちることを確認した」という担保を持たない**。緑が示すのは、**現在の実装が 7-a / 7-b / 7-d の期待（送り出しが起きずコード交換へ進まず Cookie を発行しないこと／5xx であり 401・429 と同じ値にならないこと／構成がそろっていれば 7-a〜7-c の結果にならないこと）を満たしていること**までである。**本行は 10-19 とは別の事象である** —— **10-19 が扱うのは 5-h / 5-i（Cookie に付く3属性）、本行が扱うのは 7-7（構成の欠落に対する応答の形）であり、対象も表も異なる**。**本行は限界の記録であって、要求の緩和ではない** —— **本行を根拠に AC-7-7 (i)〜(v)・7-a〜7-d の期待・AC-8-5・AC-8-2 のいずれも緩めない**。**AC-8-2 の本文（`web-app-scaffold.md` AC-3-7 の例外を本 Issue に適用しない）も、本行を根拠に読み替えない**。**あわせて本行は「限界として記録する」ところまでであり、実装・テストの作り直しを要求しない**（**これらの行について Red を踏み直す形が要ると判断したら、変える前に停止して人間へ上げる**）。**収束先（宣言を現況へ改めること）は人間が決定した**（2026-09-04。**採らなかった案 —— 「既に Red を踏んだ表へ網羅を足す場合は Red ゲートの対象外」という一般規則を明文化する（今後の全 Issue に抜け道を作るため不採用）／網羅を広げたコミットを取り消す**。記録を消さない） |
...
| 12-h | 戻りの処理が**成功した**場合／**拒否された**場合 | **いずれでも一時 Cookie が破棄されている**（5-10 (iv)。**成功側だけを検査しない**）。**構成の欠落で本来の処理へ到達しない経路（7-7 (ii)）は本行の対象外であり、その位置づけと帰結は限界 10-18 が持つ** |
