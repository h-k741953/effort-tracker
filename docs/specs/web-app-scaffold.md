...
| 9-c | **機械的な歯止めが担う範囲は限定的である**（2026-08-18 訂正。旧記述「機械的な歯止めは 11-4 のテストベクタが担う」は**事実に反していた**）。11-4 が見るのは「`paths` の正規表現が 11-4 に列挙した11本のパスをどう判定するか」だけであり、**その11本の判定を変えない弱体化は素通りする**。実測で素通りしたのは (1) `^` アンカーの削除 (2) `[[allowlists]]` の増設 (3) `[extend]` 内の `disabledRules` 追記 の3種（前掲「訂正の根拠となった実測」）。これらは **AC-12 の構造検査**で捕捉する |
...
| # | 許可する形 |
|---|---|
| 2-a | **コメント行**（`#` で始まる行）と**行末コメント**、および**空行** |
| 2-b | **テーブルは `[extend]` が1つ、`[[allowlists]]` が1つ、合計2つだけ。** それ以外のテーブル／配列テーブルを**1つも許さない**（2本目の `[[allowlists]]`、`[allowlist]`（単数形）、`[[rules]]`、`[[rules.allowlists]]`、`[allowlists.xxx]` 等を含む） |
| 2-c | **`[extend]` 内のキーは `useDefault` のみ**、値は `true` のみ。他のキー（`disabledRules` / `path` / `url` / `disabledPaths` 等）を**1つも許さない** |
| 2-d | **`[[allowlists]]` 内のキーは `paths` のみ。** 値は**要素1個**の配列で、要素は TOML の**リテラル文字列**（`'''...'''` または `'...'`）。他のキー（`regexes` / `stopwords` / `commits` / `targetRules` / `condition` / `description` 等）を**1つも許さない**（11-3-a / 11-3-e / 11-5 の「内容ベースの除外」を構造として封じる） |
...
| 3-j | 正常形の各行に**行末コメント**を付けたもの（例: `useDefault = true  # 理由`） | **合格** | 12-2-a / 2-f。誤検知しないこと |
...
| # | 要求 |
|---|---|
| 4-a | **`paths` の正規表現が `^` による先頭アンカーを持つことを検査する。** `apps/web/\.next/` というプレフィックス文字列が保持されていても、**アンカーが無ければ不合格**とする |
