...
| 7-b | **同じ `.gitleaks.toml` が `gitleaks git` にも適用されるかは、本仕様の時点で未確認である。** 設定はサブコマンド共通に読まれると見込まれるが、**実測していないので断定しない** |
| 7-c | ただし現状 `.next` は履歴に入ったことがなく（前掲の実測）、適用の有無にかかわらず `gitleaks git .` の結果は **EXIT=0 のまま変わらない**。実装工程で確かめたら、**本仕様の 7-b を実測値に置き換える**（推測のまま残さない） |
...
| 9-c | 機械的な歯止めは **11-4 のテストベクタ**が担う（範囲を広げる変更はテストを落とす）。**テストベクタごと書き換えることは機械には止められない**（AC-11-L5） |
...
| # | 内容 |
|---|---|
| 11-L1 | **`apps/web/.next/` 配下は、作業ツリー走査（`gitleaks dir`）の対象外になる。** そこへ本物の秘密が置かれても検出されない。**「安全になった」のではなく「この範囲で見なくなった」** |
...
- `docs/harness/verification-loop.md`: `make scan-secrets`（作業ツリー + 履歴）の位置づけと CI の `secrets` ジョブ（AC-11）。**gitleaks の除外範囲の正解は本仕様にある**（ADR 0004）
