...
| **採用トークンの名称と値・共通コンポーネントの一覧と props** | **AI（本仕様）** | ADR 0015 がデザイン工程へ委譲した範囲。**人間の承認を受けた決定ではない。** 人間が差し替える場合は AC-2 / AC-5 / AC-6 の表の値を置き換える（AC の構造は変えない） |

**この表の最終行は「AI が決めてよい」と読むための行ではなく、どこまでが人間の決定かを取り違えないための行である。** 業務ルール・ドメイン用語・画面の増減は本仕様では決めない（AC-7・U-1）。
...
| P-6 | Web のテストランナーは Vitest。アサーション／モックライブラリの追加可否は「必要になった Issue で判断する」として保留されている | ADR 0007 §5／`web-app-scaffold.md` AC-3-5 |
...
### U-1. コンポーネントのレンダリング検証手段

**AC-6 が要求する DOM・アクセシビリティ属性を、何で測るかが決まっていない。** `web-app-scaffold.md` AC-3-5 は「アサーション／モックのライブラリを追加しない。**『入れる』判断も本仕様の範囲外であり、必要になった Issue で判断する**」として判断を先送りしており、コンポーネントを初めて作る本 Issue がその「必要になった Issue」に当たる。**AI が独断で依存を増やさない**（技術選定は人間の決定事項。`docs/rules/responsibility.md`）。

| 案 | 内容 | 代償 |
|---|---|---|
| (a) | `@testing-library/react` + DOM 環境（`jsdom` 等）を導入し、`vitest.config.ts` で `environment` を設定する | 依存が増える。ADR 0007 が Go 側で採った「ライブラリを足さない」姿勢と Web 側の姿勢がずれる（ずれを承知で決めるなら記録が要る） |
| (b) | **依存を増やさず**、既存の `react-dom`（`react-dom/server` の静的レンダリング）で出力 HTML を検証する | クリック・キーボード操作（AC-6 の `ConfirmDialog` の Esc 等）を検証できない。検証できない項目は限界として固定する必要がある |
| (c) | DOM 検証を行わず、CSS・型・純関数の検査だけに限る | AC-6 の a11y 属性が機械検査されなくなる（規律頼みになる） |

**この決定は AC の中身を変えない**（AC-6 は観測可能な出力として書かれている）。決まるまでの間、tester は AC-2 / AC-3 / AC-4 / AC-5 の Red を先に書ける（AC-10）。
...
| # | 対象 | 手段 | U-1 に依存するか |
|---|---|---|---|
| 10-1 | AC-1 / AC-2 / AC-4-1〜4-3 | `apps/web/src/app/globals.css` を読み、トークン名・値・定義ブロック（`:root` / dark / `@theme inline`）の有無を突き合わせる | しない |
| 10-2 | AC-3 | AC-2 の表の16進値から相対輝度とコントラスト比を計算し、下限と比較する（計算は外部ライブラリを使わずテスト内で行う） | しない |
| 10-3 | AC-2-4 / AC-4-4〜4-6 / AC-5-5 | `apps/web/src/components/` 配下の実装ファイル（`*.test.tsx` を除く）を読み、禁じた表現（生の色・`rounded-md` 以外の角丸・代替なしの `outline-none`・`fetch`）の不在を検査する | しない |
| 10-4 | AC-5-1 / AC-5-2 | `apps/web/src/components/` 直下の **`*.tsx`（`*.test.tsx` を除く）の集合**が表と**過不足なく**一致することを検査する（表に無い `*.tsx` の存在も違反。テストファイルと `.gitkeep` 等の非 `*.tsx` は対象外）。あわせて **`index.ts` / `index.tsx`（バレル）が存在しないこと**を検査する | しない |
| 10-5 | AC-6 の props 契約（型） | `npx tsc --noEmit`（`make lint-web` が呼ぶ）。禁じた props を渡すコードが型エラーになることを、`@ts-expect-error` を用いた型テストで示す | しない |
| 10-6 | AC-6 の出力・a11y 属性 | レンダリング結果の検証。**手段は U-1 の決定に従う** | **する** |
| 10-7 | AC-7 / AC-8 / AC-9 | レビューでの突き合わせ（機械検査しない。AC-11） | しない |
...
| 11-7 | **U-1 が未決の間、AC-6 の出力側（a11y 属性・ラベル文字列）は Red を踏めない**（AC-10-6）。**「tester が確認できない」は「誰も確認しなくてよい」を意味しない** —— U-1 が決まるまで当該部分の実装に入らない（`web-app-scaffold.md` AC-3-7 と同じ扱い。ただし本 Issue はランナー導入 Issue ではないため、同 AC-3-7 の例外は適用しない） |
