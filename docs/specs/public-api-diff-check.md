...
| AC-3 | 抽出規則 — 何を公開 API と数え、どう正規化するか（**抽出器自身の扱いを含む** — 3-15 / 3-16） | tester・implementer |
...
- **ADR 0018 には手を入れない**（ADR 0001 運用ルール3「ADR は書き換えず、追記もしない」）。**本仕様も新しい ADR を起こしていない。** 決定6 が丸ごと覆ったのか、決定6 が決めていなかった詳細を埋めただけなのかの線引きは**人間の判断事項として未処理のまま残っている**（AC-9-14）
- **ハーネス資産（チェッカ）がプロダクトモジュール `services/api` の中に入る。** ADR 0008 が定めるディレクトリ構成に、この用途の `cmd/` は無い。その直接の帰結として**抽出器自身が抽出対象に入る**（自己言及）。扱いは **AC-3-15 / AC-3-16** で確定させている
...
| 3-16 | 3-15 は**抽出器の Go テストが機械的に固定する**: 抽出器のパッケージディレクトリ自身へ本 AC の抽出規則を適用し、**得られるレコードが0件であること**を検査する。このテストは `make test-api`（`go` ジョブ）の対象であり、**比較器の shell fixture（AC-6）には持ち込まない**（AC-1-3） |
...
| 9-14 | **本仕様は ADR 0018 決定6 の字面から外れる**（相乗り先を `scripts` ではなく `go` ジョブとする。D-1 の「代償」）。**ADR は書き換えず、追記もせず、新しい ADR も起こしていない**（ADR 0001 運用ルール3）。したがって **「決定6 が丸ごと覆ったのか、決定6 が決めていなかった詳細を埋めただけなのか」の線引きは未処理のまま残っている。これは人間の判断事項であり、本仕様が決めてよい範囲ではない** |
