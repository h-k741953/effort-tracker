...
### D-2. fixture ターゲットも `go` ジョブへ置く（本仕様が決めた事項）

**決定者は D-1 と異なる。** D-1 が決めたのは警告 step のジョブであり、`make test-public-api-diff`（比較器の fixture）を CI のどのジョブが呼ぶかは決まっていなかった。**本仕様はこれを `go` ジョブとする。**
