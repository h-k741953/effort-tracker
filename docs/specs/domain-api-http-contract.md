...
> **実装設計側の論点も確定済み**（`workmonth-implementation-design.md`「人間が確定させた決定」。集約が技術者を持つか・UC1 の操作名・Postgres ドライバ・契約期間の検査は 2026-07-27、当該年月に属さない対象日への削除は 2026-07-28）。**ただし 2026-07-30 に UC2 の実装設計から1件が未決として上がっている**（同ファイル「人間の決定を待っている論点」Q-1＝復元時の超過／不足と状態の整合検査）。**Q-1 は集約の復元に閉じ、本契約の表現・ステータス・判定順序を変えない。****Postgres ドライバは pgx に確定し、ADR 0017 を起票した**（本契約には影響しない。HTTP の表現は変わらない）。
