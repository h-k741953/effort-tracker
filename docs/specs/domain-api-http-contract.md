...
- **決定者**: HTTP 表現の各決定は仕様工程（AI）。**業務ルールを新たに決めていない。** 業務仕様が答えを持たなかった論点（未生成の年月の締め・参照系の認可・操作者の伝達）は **2026-07-27 に**、当該年月に属さない対象日への削除は **2026-07-28 に**、いずれも**人間が明示選択して確定**し、末尾「人間が確定させた決定」に残した。**未決の論点は無い**
- **日付**: 2026-07-27（人間の決定事項を反映）／**2026-07-28 追記**（D-13・AC-5-8。`daily-record-entry.md` D-9 の HTTP 表現）／**2026-07-30 追記**（AC-6-9。締めで複数条件が同時に該当する場合の判定順序を明示。**新しい決定ではなく AC-9 の順序の当てはめ**）
...
| D-13 | 当該年月に属さない対象日への**削除**（E-4） | **400 `DATE_OUT_OF_WORK_MONTH`** で弾く（入力・編集の AC-4-7 と同じ表現に揃える）。**業務ルールの正解は `daily-record-entry.md` D-9・AC-5-5 が持ち**、本契約が定めるのはその違反の HTTP 表現（ステータス・`code`・判定順序上の位置＝AC-9 の順 6「業務バリデーション」）だけである。**未生成の年月への削除を 200 とする AC-5-3 の扱いより優先する**（未生成でも 400）。**人間が 2026-07-28 に確定**（それ以前の実装は 200 no-op だった） |
...
| # | 条件 | 期待 |
|---|---|---|
| 5-1 | 生成済み・`Draft`・本人・`{date}` の年月が `{yearMonth}` と一致・当該日にレコードあり | **200**。レコードを取り除き、更新後の勤務月の表現を返す。その日は以降「レコードのない日＝稼働なし」（`daily-record-entry.md` AC-5-4・D-5） |
| 5-2 | `{date}` の年月が `{yearMonth}` と一致し、当該日にレコードが無い | **200**（成功として扱う）。「レコードのない日」と「稼働ゼロ」を区別しない業務決定（同 D-5）の帰結であり、削除後の観測可能な状態は同一である。**`{date}` の年月が一致しない場合はこの行を適用しない**（5-8） |
| 5-3 | 未生成の `契約 × 年月`（`{date}` の年月は `{yearMonth}` と一致） | **200**。勤務月を**生成せず**、空の表現（`generated: false`）を返す（明示的な生成契機を作らない。P-5）。**削除は D-11（未生成なら 404）の対象外**である。削除の観測可能な結果は 5-2 と同じ「その日にレコードが無い」であり、404 にする理由が無いため。**`{date}` の年月が一致しない場合はこの行を適用しない**（5-8 が優先し 400） |
...
| 5-8 | **`{date}` の年月が `{yearMonth}` と一致しない**（例: `/work-months/ctr-0001/2026-07/daily-records/2026-08-01`） | **400 `DATE_OUT_OF_WORK_MONTH`**（`daily-record-entry.md` AC-5-5・D-9。D-13）。**当該日にレコードがあるか否か、勤務月が生成済みか否かを問わず 400**（5-1〜5-3 に優先する）。入力・編集の AC-4-7 と同じ `code`・ステータス |
| 5-9 | 5-8 と他の条件が同時に該当する場合 | **AC-9 の判定順序に従う。** 5-8 は**順 6（業務バリデーション）**であり、認証(1)・構文(2)・存在(3)・認可(4)・状態(5) の後に判定される。したがって、ゲスト → **401**（AC-1-6）、`{date}` が暦上存在しない／書式不正 → **400 `INVALID_REQUEST`**、実在しない `contractId` → **404**（5-6）、本人でない → **403**（5-4）、`PendingApproval` / `Approved` → **409**（5-5）が**先に**返る |
...
| 6-9 | 複数の条件が同時に該当する場合 | **AC-9 の判定順序に従う。** ゲスト → **401 `UNAUTHENTICATED`**（順1。AC-1-6）、`yearMonth` の書式不正 → **400 `INVALID_REQUEST`**（順2）、実在しない `contractId` → **404 `CONTRACT_NOT_FOUND`**（順3）、未生成 → **404 `WORK_MONTH_NOT_FOUND`**（順3。6-6。**操作者が本人でなくても 404 が先に返る**）、本人でない → **403 `FORBIDDEN_NOT_OWNER`**（順4。6-3）、`Draft` 以外 → **409 `INVALID_STATE_FOR_CLOSE`**（順5。6-2）。**締めには順6（業務バリデーション）に当たる判定が無い**（未入力日・空月を弾かないため。6-5・`monthly-closing.md` D-1・D-6） |
...
| `DATE_OUT_OF_WORK_MONTH` | 400 | 対象日が当該勤務月の年月に属さない。**入力・編集（AC-4-7）と削除（AC-5-8）の双方**で返す（削除では勤務月の生成有無・当該日のレコードの有無を問わない） | 同 AC-2-4（入力・編集）／同 AC-5-5・D-9（削除） |
...
| 6 | 業務バリデーション（値域・未来日・当該月外） | 本人が `Draft` に 25時間0分を入力 → 400。本人が `2026-07` の `Draft` に対し `2026-08-01` の削除を要求 → 400 `DATE_OUT_OF_WORK_MONTH`（AC-5-8。**勤務月が未生成でも 400**。ただし同じ要求をゲストが行えば 401、他人が行えば 403、`Approved` に対して行えば 409 が先に返る） |
...
業務仕様が答えを持たず、**推測で埋めなかった**論点は、**2026-07-27 および 2026-07-28 に人間が明示選択して確定した**（Issue #51）。**未決の論点は残っていない。** 確定内容は上表の D と各 AC に反映済みであり、下表は「何を選び、何を退けたか」を残すためのものである（Issue コメントに書き写さない。ADR 0004）。
...
| 4 | **当該年月に属さない対象日への削除**（2026-07-28 確定）。`daily-record-entry.md` は AC-2-4 で入力側だけを弾いており、削除側の規則が無かった。それ以前の実装は 200 no-op だった | **400 `DATE_OUT_OF_WORK_MONTH` で弾く。** 集約の不変条件（勤務月は当該年月の実績のみ保持）と一致し、`Reconstruct` が同じ日付を弾くこと（実装設計 AC-2-5）とも整合する。200 だとクライアントのバグが黙って隠れる | (a) 200 no-op のまま（「レコードのない日への削除は成功」＝ `daily-record-entry.md` D-5 の延長として扱う） (b) 404 を返す（対象日を独立したリソースとみなす） | D-13・AC-5-8・AC-5-9・AC-9（エラー表・判定順序 順6）／**`daily-record-entry.md` D-9・AC-5-5**（業務ルールの実体）／実装設計 AC-2-5・AC-3-11・AC-4-2・AC-7-9 |

> **実装設計側の論点も確定済み**（`workmonth-implementation-design.md`「人間が確定させた決定」。集約が技術者を持つか・UC1 の操作名・Postgres ドライバ・契約期間の検査は 2026-07-27、当該年月に属さない対象日への削除は 2026-07-28）。**2026-07-30 に UC2 の実装設計から上がっていた1件（復元時の超過／不足と状態の整合検査）も 2026-07-31 に確定し**（同ファイル決定9＝検査する。違反は `ErrInvalidValue`）、**実装設計側にも未決の論点は残っていない**。**この決定は集約の復元に閉じ、本契約の表現・ステータス・判定順序を変えない**（対象は**正常な経路では作られない永続化行**のみで、利用者の要求の不正ではない。外向きには AC-9 のエラー表の `INTERNAL_ERROR`＝500「永続化の失敗等」に含まれ、400 のいずれの `code` にも割り当てない）。**Postgres ドライバは pgx に確定し、ADR 0017 を起票した**（本契約には影響しない。HTTP の表現は変わらない）。
