...
- **非対象**（未確定ではなく、確定済みで別 spec が持つ）:
  ...
  - デザイントークン・配色・タイポグラフィ・レスポンシブ・具体レイアウト（ADR 0015 のデザイン工程。受け入れ条件は `design-system.md`）
  - 業務ルールそのもの（各業務仕様が持つ）
...
| 8-2 | **デザイントークン・具体レイアウト・レスポンシブは対象外**（ADR 0015 のデザイン工程が担い、受け入れ条件は `design-system.md` が持つ）。本仕様は受け入れ条件（満たすべき形）のみを定める |
