...
| 12-16 | **`driver/persistence` のテスト**（AC-10-10〜AC-10-14。**実 DB を要さない範囲に限る**。**2026-08-10 追記**） | **実 DB を使うテストの方針は本仕様では決めない**（AC-12-6・AC-13-4 は不変）。**テストは pgx を import しない**（AC-12-6・ADR 0017）。したがって**手書きの偽 pgx（pgx のインターフェースを満たすテストダブル）も作らない**。この制約の下で**固定するのは次の3つだけ**である（**2026-08-13 に ③ を追加**＝決定15。**①②は変更していない**）。①**設定の組み立て**（AC-10-12）— 環境変数の探索を**引数で差し替えた**状態で、(i) **必要な設定が未設定・空なら、接続を試みずにエラーを返す**こと、(ii) **必要な設定が揃っていればエラーにならず設定が組み立つ**こと（**(i) と対にして置く**。対にしないと「常にエラーを返す」実装が Green になる）、(iii) **エラーの文言に対する漏洩の検査は、本パッケージ（`driver/persistence`）では行わない —— 置き場所はエントリポイントのパッケージ（`cmd/bootstrap`）へ移した**（**2026-08-27 訂正**）。**移した理由**: **2026-08-26 の AC-10-12 の訂正により、設定の組み立ては接続文字列を探索から読まなくなった**（取得済みの値を引数として受け取る形＝`docs/specs/infra-terraform.md` AC-8-6）。その結果、**当該層に「探索で得た値」が存在しなくなった**。ここに検査を残しても**空回りする** —— **設定の組み立ての失敗経路は (i) の「必要な設定が未設定・空」の1本だけ**であり、**空の値は漏れようがなく、非空なら成功してエラーが返らない**。したがって**どんな実装に対しても緑になる検査**にしかならず、Green が何も検証しない（`docs/harness/verification-loop.md`）。**移設先の要求の持ち主は `docs/specs/infra-terraform.md` AC-8-9 である** —— **何をエラーの文言・ログに含めてはならないかの本文はそちらが持ち、ここへ書き写さない**（ADR 0004。テストの置き場所としてはエントリポイントのパッケージ＝AC-12-17 ③ と同じ位置づけ）。**本行は漏洩を許す方向へ緩める趣旨ではない** —— **移るのは検査の置き場所だけ**であり、**`driver/persistence` が自ら構築するエラーの文言に対する要求（AC-10-13 ③・`docs/rules/security.md`）は不変**である。**既存の検査の置き場所を動かす訂正であるため、人間の承認事項である**（`docs/rules/development-process.md`「要求を実装の都合で緩めない — 緩める判断は人間の承認事項」）。**承認済み: 人間、2026-08-27。** したがって本行の範囲に限って改めてよく、**これを根拠に ① の (i)(ii) や ②③ の要求を緩めない**。**テストは環境変数の名前に依存しない形で書く**（AC-10-12 で名前を固定していないため。差し替えた探索が要求された名前を記録するだけにし、名前そのものを期待値にしない＝AC-12-10 と同じ形）。**プロセスの環境変数を書き換えない**（AC-10-12 の「差し替えられる形」を観測する唯一の手段であり、書き換えないと Red にならない）。②**`gateway` が要求する形への適合**（AC-10-14。**2026-08-10 確定＝決定14**）— pgx 実装が AC-9-14-e の3 + 4 + 4 のメソッドを満たすことを**コンパイル時に**検査する: **`adapter/gateway` を import し**（決定14）、**gateway が宣言したインターフェース型の変数へ pgx 実装を代入する**（`var _ <gateway が宣言する型> = <pgx 実装>` の形。**型名は固定していない**ため、テストは gateway 側から型を引き、名前を期待値にしない＝AC-12-10 と同じ形）。対象は **`DB` 相当・`Rows` 相当・`Tx` 相当の3つすべて**とする（`DB` だけでは、`Query` / `Begin` の戻り値として返す実装が `Rows` / `Tx` を満たさない取り違えが検出できない）。**メソッドが欠けている・署名が異なれば、Red はテストの失敗ではなくビルドの失敗として現れる**（AC-13-12 と同型。「存在しないもの」を呼ぶ検査と同じく、Red がコンパイル不能として観測される）。**別名の interface を `driver/persistence` 側に宣言してそれへ代入しない**（AC-10-14。それでは `gateway` が要求する形への適合を示さず、常に Green になる）。③**同時に開く接続の本数**（AC-10-19。**2026-08-13 追記＝決定15**）— **実 DB も pgx も要さず**（①と同じ制約。**手書きの偽 pgx も作らない**）、次の3つを固定する: (i) **本数の出所（AC-10-19 ②）を読み、その値が 1 であること** — **期待値の 1 はテスト側に直書きし、実装側の値どうしを突き合わせない**（突き合わせるとどの値でも Green になる）。**出所を 1 以外へ変えると Red**。(ii) **組み立てた接続設定に本数の指定が載り、その値が 1 であること**（AC-10-19 ③） — ① と同じ形（**環境変数の探索を引数で差し替え、必要な設定が揃った状態**）で設定を組み立て、その結果から**標準ライブラリだけで**本数の指定を読み取る。**指定が載っていない（＝ライブラリの既定値へ落ちる）実装、および 1 以外が載る実装では Red**。**テストは `driver/persistence` の内部テストパッケージ（`package persistence`）に置くため、非公開のフィールド・識別子をそのまま読んでよい**（**本数を読み取るためだけに公開のアクセサを足さない**。接続文字列そのものを取り出す手段を公開しないという既存の扱い＝AC-10-13 ③・`docs/rules/security.md` を変えないため）。**(i) と対にして置く**（(i) だけでは、出所の値を確立の手段が使っていない実装が Green になる）。(iii) **探索が返す値に本数が依存しないこと**（AC-10-19 ④） — ① と同じく**どの名前に対しても同一のダミー値を返す探索**（**テストは環境変数の名前に依存しない形で書く**＝① と同じ理由）で組み立てても、(ii) の読み取り値が **1** であること。**本数を環境変数から読む実装では、ダミー値から 1 が得られず Red になる**（**探索を読んだうえで 1 へ落とす実装は Green のまま残る**＝AC-13-25 ④）。**プロセスの環境変数を書き換えない**（① と同じ）。**③ が示すのは「設定の組み立てまでで本数が 1 に決まっていること」までであり、pgx が実際に張る接続の本数・Neon 側で開かれる本数は示さない**（AC-13-25）。**①②③のいずれも、pgx の戻り値を `gateway` の形へ写す部分の正しさ・接続・トランザクションを観測しない**（AC-13-23） |
