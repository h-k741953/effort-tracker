...
| 1-7 | **エントリポイント（`cmd/…`。`package main`）**（**2026-08-10 追記**。D-12） | import してよいのは **`driver/lambda`・`driver/persistence`・標準ライブラリ・Lambda ランタイムの登録に要るもの**（`aws-lambda-go`）**と、SQL 実行インターフェースの型を名指すための `adapter/gateway`**（AC-10-18 の引数の型。`driver` → `adapter` と同じ内向きで、決定14 と同じ理由）**だけ**。**`usecase/*`・`domain` を直接 import しない**（結線は `driver/lambda` の AC-10-16 に集約する）。**pgx を import しない**（AC-1-6・D-11 は不変） |

**配置**（ファイル名は例示。本 AC が固定するのは**パッケージと責務の対応**）:

**`internal/` の外**（**2026-08-10 に1つだけ追加**。既存の `internal/` の配置は変更していない）:

```
services/api/
└── cmd/<実行ファイル名>/    エントリポイント（package main）。設定の取得・接続の確立・ランタイムへの登録（AC-10-15。葉の名前は固定しない＝D-12）
```
...
| 10-11 | **接続の確立と所有**（AC-10-9 と対。**2026-08-10 追記**） | 確立の**手段**（関数）を公開するが、**確立そのものをパッケージの初期化で行わない**: `init()`・パッケージ変数での接続確立・プロセス内シングルトンを**持たない**。**いつ確立し、いくつ保持し、再利用するか**（コールドスタート時に1度だけ＝AC-10-2）を決めるのは**呼び出し側**（AC-10-8 ③ の呼び出し側＝AC-10-9）であり、`driver/persistence` は確立済みの実装を返すところまでを担う。**確立の実行を AC-10-8 ③ の内側へ持ち込まない**（AC-10-9。③ は確立済みの実装を引数で受け取る）。**関数名・型名・戻り値が接続プールか単一接続か・接続を閉じる手段を公開するかは固定しない**（AC-13-17 と同じ扱い。`pgxpool` を使うかも ADR 0017 が実装 PR へ委ねた範囲）。**2026-08-10 追記: 「いくつ保持するか」を予約同時実行数（`docs/rules/cost-guardrails.md`）と掛け合わせた本数として決める判断は Q-6 として起票済みで未決**である。**本行は Q-6 の確定前も成立する**（本数を固定していないため）が、**確定したら本行と AC-10-18 に反映する。仕様工程が本数を決めない** |
...
>
> **HTTP 契約側の論点も確定済み**（`domain-api-http-contract.md`「人間が確定させた決定」。1〜3 は 2026-07-27、4 は 2026-07-28）。**本ファイル・HTTP 契約のいずれにも未決は無い**（決定9 は集約の復元＝実装設計に閉じ、HTTP の表現を変えない）。**これは 2026-07-31 時点の記述である。2026-08-05 に adapter 層の実装設計から Q-2〜Q-4 を起票したが、いずれも同日に確定して決定10〜決定12 へ移した**ため、**現在の未決は本ファイル・HTTP 契約のいずれにも無い**（次節）。**HTTP 契約側は起票から確定まで一貫して未決なし**であり、Q-2 は契約の変更を選択肢 (b) に含んでいたが**その案は退けられ、契約は変更していない**（決定10）。**2026-08-10 に `driver/persistence` の受け入れ条件を起こした際に Q-5 を起票したが、同日に人間が明示選択して確定し決定14 へ移した**ため、**現在の未決は本ファイル・HTTP 契約のいずれにも無い**（次節。**HTTP 契約側は起票から一貫して未決なし**であり、**決定14 も HTTP の表現を変えないため契約を変更していない**）。**その後、同日にエントリポイントを起こした際 Q-6（`driver/persistence` が張る接続の本数と予約同時実行数の掛け算）を起票したため、現在の未決は本ファイルの Q-6 の1件である**（次節。**HTTP 契約側は引き続き未決なし**。**Q-6 も HTTP の表現を変えないため契約に触れない**）。
>
...
**現在の未決は Q-6 の1件である**（2026-08-10 起票）。
...
### Q-6. エントリポイントが張る接続の本数（**未決**。2026-08-10 起票）

**論点**: `driver/persistence` が公開する接続の確立の手段（AC-10-11）は**接続プールを張り、その本数を調整する手段を公開 API に持たない**。`docs/rules/cost-guardrails.md` が定める **Lambda の予約同時実行数**と掛け合わせると、**Neon 側で同時に開かれる接続の本数がその積になり得る**。**AC-10-11 は「接続プールか単一接続か」を固定していないため現状は違反ではない**が、**コストとセキュリティのガードレールは機能要件と同格**（CLAUDE.md）であり、**接続の本数を決めることは `docs/rules/cost-guardrails.md` の側に属する判断**であるため、**仕様工程が推測で埋めない**。

**なぜ今上げるか**: エントリポイント（AC-10-15・AC-10-18）が**接続の確立を呼ぶ唯一の場所**になり、「いくつ保持するか」を決める責務がここに集まったため（AC-10-11 が「いつ確立し、いくつ保持し、再利用するかを決めるのは呼び出し側」と定めている）。

**選択肢**:

- **(a) 本数の上限を明示的に固定できる形にする** — 接続の確立の手段が上限を受け取れるようにし（AC-10-12 の設定の一部として扱うか、確立の引数として渡すかも含めて決める）、**上限 × 予約同時実行数**が Neon 側の許容に収まる値を**人間が決める**。値は `docs/rules/cost-guardrails.md` の表へ書き、本仕様は AC 番号で参照する（ADR 0004）。
- **(b) 実行あたり1接続にする**（プールを使わない、または上限を1にする）— Lambda は1つの実行環境が同時に1要求しか処理しないため、**同時実行数と接続本数が1対1に対応**し、上限が予約同時実行数だけで決まる。**推奨はこれ**（理由: 値を2箇所で管理せずに済み、`docs/rules/cost-guardrails.md` の予約同時実行数が唯一の実体のままになる。ただし**実行のたびに接続確立の往復が増える可能性**があり、AC-10-2 の「1度だけ確立して再利用」とは**実行環境の寿命の中で**両立させる必要がある）。
- **(c) 現状（プールの既定値）のまま据え置き、限界として記録する** — 本数が実行環境の資源に依存し、**Neon 側の上限に達するかどうかを実接続でしか確かめられない**（AC-13-23 ⑤ と同じ性質）。**この案を採る場合も、据え置いたこと自体を AC-13 の行に記録する**。

**決まるまで進めないこと**: 本仕様は**接続の本数・上限・その設定手段を定めない**。**AC-10-11（接続プールか単一接続かを固定しない）と AC-10-18（確立をちょうど1回・要求ごとに実行される位置に置かない）は Q-6 の確定前でも成立する**ため、**実装・テストは AC-10-15〜AC-10-18・AC-12-17 の範囲で先に進めてよい**。確定したら、反映先は **AC-10-11・AC-10-18 と、値を持つ `docs/rules/cost-guardrails.md`**（値の実体は本仕様に書かない＝AC-13-1・ADR 0004）。
