...
- **決定者**: 実装設計の各決定は仕様工程（AI）。ただし**集約の中身・ユビキタス言語への語の追加・技術選定・業務ルールに関わる決定は人間の決定事項**であり（`docs/ai-collaboration.md` 責務分界表）、該当する行の「決定者」欄には **人間** と記す。2026-07-27・2026-07-28・2026-07-31 に上げた論点は**すべて確定済み**で、経緯と確定内容は末尾「人間が確定させた決定」に残す。**2026-08-05 に adapter 層の実装設計から Q-2〜Q-4 を起票しており、未決の論点がある**（末尾「人間の決定を待っている論点」。推測で埋めていない）
...
> **操作者ヘッダ不在（ゲスト）と本表の検査が同時に該当する場合の順序は Q-2（未決）である。** 契約 AC-9 は順1（認証）が順2（構文）に先行すると定めるが、controller が入力 DTO を組み立てられない要求は interactor（順1 を判定する層＝AC-8-7）へ到達しない。**確定するまでこの組み合わせのテストを書かない。**
...
| # | 条件 | 期待 |
|---|---|---|
| 9-7-a | 両ヘッダ不在 | **未認証の `port.Actor`**（`Authenticated` が偽）を組み立てて渡す。`Guest` というロール値を作らない（D-9） |
...
| # | 条件 | 期待 |
|---|---|---|
| 9-8-a | 依存の向き | controller は **`usecase/interactor` を import しない**（AC-1-5 の許可リストに無い）。呼び出し先は**差し替え可能な抽象**として受け取り、`driver/lambda` が結線する。**抽象の置き場所は Q-3（未決）** |
| 9-8-b | 戻り値 | 使わない（interactor は戻り値を返さない。AC-7-3）。応答は presenter が保持する結果から `driver/lambda` が組み立てる（AC-9-13） |
| 9-8-c | 差し替え可能性 | いずれの選択でも、**Lambda・DB・interactor の実体を起動せずに「HTTP 要求 → 入力 DTO」の写しを検査できる**こと（AC-12-5・AC-12-9） |

**AC-9-9. 要求側の失敗を表す識別子**

| # | 条件 | 期待 |
|---|---|---|
| 9-9-a | 識別子 | controller / `driver/lambda` が検出する失敗（要求の構文・型・書式の不正、未定義のパス・メソッド）は、**`domain` / `usecase/port` の番兵とは別の識別子**で表し、`errors.Is` で区別できること（AC-11-9） |
...
| 9-9-c | 帰結 | controller は値オブジェクトの構築失敗を**要求側の識別子へ変換してから**出力側へ渡す。写像は AC-11-13 |
| 9-9-d | 置き場所 | **固定しない**（AC-13-17）。テストが固定するのは「controller の構文失敗は `INVALID_REQUEST` へ、`workmonth.ErrInvalidValue` は `INTERNAL_ERROR` へ写る」ことである（AC-12-10） |
...
| 9-13-c | 一度も呼ばれない場合 | 「結果なし」を表現でき、`driver/lambda` はそれを **`INTERNAL_ERROR`** として応答する（配線の誤りを 200 で返さない） |
...
| 9-14-e | 具体的な形 | **未決（Q-4）。** メソッドの粒度・行の走査の受け取り方・トランザクションの取得方法は一意に読み取れない。**確定するまで gateway の実装とそのテスト（AC-12-11）に入らない**（AC-13-17） |
...
| 12-9 | `adapter/controller` のテスト — **入力 DTO の写しの検査**（AC-12-5 が定める「Lambda を起動しない」ことに加えて、**呼び出し先の差し替え可能性をテスト側から要求する**） | controller は呼び出し先（interactor に相当するもの）を**差し替え可能な抽象として受け取る**こと（AC-9-8-a）。テストは**受け取った入力 DTO を記録するだけの手書きの spy**（AC-12-2・AC-12-3 と同じ形。モックライブラリを入れない＝ADR 0007）を渡し、`httptest` で組み立てた要求（AC-12-5）に対して **AC-9-5 の写し先どおりの入力 DTO がちょうど1回渡ること**を go-cmp で比較する。**Lambda・DB・interactor の実体をいずれも起動しない**（AC-9-8-c）。固定する対象は **AC-9-5（写し先）・AC-9-6（弾くもの／弾かないもの）・AC-9-7（操作者ヘッダ → `port.Actor`）** の3つで、**controller が弾いた要求では spy が1回も呼ばれていない**ことを記録で確認する（逆に、controller が弾かないと定めた値＝**当該年月に属さない対象日**（AC-9-6-d）・**値域外の稼働時間**（AC-9-6-e）・**未来日**（AC-9-6-f）は、弾かれずに**そのまま入力 DTO へ渡ること**を確認する。状態・認可は controller の入力に現れないため観測しない）。**抽象の置き場所（Q-3。AC-9-8-a）が未決でも本 AC のテストは書ける**（置き場所ではなく差し替え可能性だけに依存するため）。ただし **AC-9-6 の注記が指す組み合わせ（操作者ヘッダ不在 × 構文検査の同時該当＝Q-2）のテストは書かない**（未決を踏み越えない） |
| 12-10 | `adapter/presenter` のテスト — **エラーの写像**（AC-9-9-d が固定を求めるもの） | AC-11-13 の写像を presenter のテストで固定する: **controller が返す要求側の識別子（AC-9-9-a）は `INVALID_REQUEST` へ**、**`workmonth.ErrInvalidValue` は `INTERNAL_ERROR` へ**写ること（AC-9-9-b・AC-9-9-c）。テストは **HTTP サーバも interactor も起動せず**、presenter の `PresentError` を直接呼び、保持された結果（ステータス + ボディ。AC-9-13-b）を検査する。テーブル駆動とし、**ラップされたエラー**（`fmt.Errorf` の `%w`）でも同じ `code` へ写ることを併せて置く（写像を `errors.Is` で引くこと＝AC-9-12-a・AC-11-9 を固定するため。`==` 比較の実装では Red になる）。**対応表のいずれにも該当しないエラーが `INTERNAL_ERROR` になること**（AC-9-12-a・AC-11-11）と、**`message` に受け取ったエラーの文字列がそのまま入らないこと**（AC-9-12-b）も同じ形で固定する。**識別子の名前と置き場所（AC-9-9-d・AC-13-17）に依存しない検査**とし、テストは写像だけを観測する |
| 12-11 | `adapter/gateway` のテスト | **Q-4（gateway が宣言する SQL 実行インターフェースの具体的な形）が確定するまで着手しない。** AC-9-14-e が「確定するまで gateway の実装とそのテストに入らない」と定めており、**Fake の形がインターフェースの形に従属する**ため、確定前に書いたテストは確定後に必ず書き直しになる（推測で形を決めない）。**確定後に固定するのは次の3つ**で、いずれも**実 DB を要さない**（実 DB を使うテストの方針は本仕様では決めない＝AC-12-6・AC-13-4。**Fake も pgx を import しない**＝AC-12-6・D-11）: ①**行 → 集約の変換**（AC-9-15）— 確定したインターフェースの手書き Fake が返す行に対し、`Reconstruct` へ渡る引数が AC-9-15-b のとおりであること（精算幅は**行の値**であり契約の現在値で埋めない）、超過／不足の NULL ↔ 未確定が1対1であること（AC-9-15-c。**gateway 側で再計算も補完もしない**）、`Reconstruct` の失敗を握り潰さず呼び出し元へ返すこと（AC-9-15-d）、行が無ければ `port.ErrWorkMonthNotFound` を返し空の集約を作らないこと（AC-9-15-e）。②**集約 → 行の書き込み**（AC-9-16）— Fake が記録した書き込みが、削除された対象日の行を残さないこと（AC-9-16-b）、稼働実績が**入力された稼働時間**のみで丸め値・総稼働時間の列を持たないこと（AC-9-16-c）、超過／不足の未確定が NULL に写ること（AC-9-16-d）。③**エラーの変換**（AC-9-19）— 「行が無い」だけを `port` の番兵へ変換し、それ以外のドライバ由来のエラーはそのまま返すこと（AC-9-19-a）。**原子性（AC-9-16-a）・`ORDER BY` による並び順（AC-9-18-c）・N+1 の不在（AC-9-18-b）・SQL 文そのものの正しさは Fake では観測できず、本 AC はこれらを担保しない**（AC-13-4） |
...
| 13-17 | **adapter 層の内部の形を一部固定しない。** ①**controller / `driver/lambda` が検出する失敗を表すエラー識別子の名前と置き場所**（AC-9-9-d）— 本仕様が固定するのは**写像だけ**（要求側の識別子 → `INVALID_REQUEST`、`workmonth.ErrInvalidValue` → `INTERNAL_ERROR`。AC-11-13）であり、どのパッケージにどの名前で置くかは実装 PR に委ねる。テストも名前と置き場所に依存しない形で書く（AC-12-10）。②**gateway が受け取る SQL 実行インターフェースの具体的な形**（メソッドの粒度・行の走査の受け取り方・トランザクションの取得方法。AC-9-14-e）— こちらは**未決（Q-4）**であり、確定するまで gateway の実装とそのテストに入らない（AC-12-11）。**①と②は性質が異なる**: ①は「固定しないと決めた」ものであり実装 PR が自由に選べるが、②は「**決められないまま残っている**」ものであり、推測で形を決めて先に進めてはならない |
...
| 13-19 | **`driver/lambda` のルーティングと DI 配線を検査するテストを本仕様は要求しない。** AC-12 の各テストは層ごとに手書きの spy / Fake で閉じており（AC-12-5・AC-12-9・AC-12-10・AC-12-11）、**Lambda も実 DB も起動しない**。帰結として、**パスと controller の対応の誤り（AC-10-1）・gateway / presenter / `Clock` / interactor の結線の誤り（AC-10-2）・presenter が一度も呼ばれない配線（AC-9-13-c の「結果なし」）**は、どの層の単体テストでも Red にならない。担保はレビューと、デプロイ後の経路の確認に留まる |

---

## 人間が確定させた決定

**`docs/ai-collaboration.md` の責務分界表で人間の決定領域**（集約の切り方・ユビキタス言語の用語・技術選定・業務ルール）に触れる論点は、**2026-07-27・2026-07-28・2026-07-31 に人間が明示選択して確定した**（Issue #51）。確定内容は上表の D と各 AC に反映済みであり、下表は「何を選び、何を退けたか」を残すためのものである（Issue コメントに書き写さない。ADR 0004）。**未決の論点は Q-2〜Q-4**（2026-08-05 に adapter 層から起票。次節）。
...
>
> **HTTP 契約側の論点も確定済み**（`domain-api-http-contract.md`「人間が確定させた決定」。1〜3 は 2026-07-27、4 は 2026-07-28）。**本ファイル・HTTP 契約のいずれにも未決は無い**（決定9 は集約の復元＝実装設計に閉じ、HTTP の表現を変えない）。**これは 2026-07-31 時点の記述である。2026-08-05 に adapter 層の実装設計から Q-2〜Q-4 を起票しており、現在の未決は次節にある**（HTTP 契約側は依然として未決なし。Q-2 は契約の変更を選択肢に含むが、契約はまだ変更していない）。
>
...
**2026-08-05、adapter 層の実装設計から Q-2〜Q-4 を起票した。** いずれも既存 AC と `domain-api-http-contract.md` から**一意に読み取れず**、埋めるには人間の決定領域（契約の変更・ポートの増減に当たる設計変更）に触れる。**推測で埋めていない**（`docs/ai-collaboration.md`「AIの停止条件」）。本文の該当 AC には未決である旨を記し、確定するまで踏み越えない範囲も示してある。

**Q-2. 操作者ヘッダ不在（ゲスト）と controller の構文検査が同時に該当する場合の順序**

- **一意に読み取れない点**: 契約 AC-9 は**順1（認証）が順2（構文）に先行する**と定める。しかし controller が入力 DTO を組み立てられない要求（契約識別子・年月・対象日の書式不正など＝AC-9-6-a〜AC-9-6-c）は、**順1 を判定する層＝interactor（AC-8-7）へ到達しない**。したがって「操作者ヘッダが無く、かつ構文も不正」な要求に対して、`UNAUTHENTICATED` と `INVALID_REQUEST` のどちらが返るのかが、契約からも本仕様からも一意に決まらない。
- **選択肢**:
  - (a) **controller が順1 を再現する** — 操作者ヘッダの有無の判定を構文検査より前に置き、未認証なら DTO を組み立てずに認証エラーとして返す。契約 AC-9 の順序は保たれるが、**認証の判定材料が controller にも現れる**（認可は interactor＝D-4 という分担との関係を要確認）。
  - (b) **構文検査を先に返すことを契約側で明示する** — 契約 AC-9 に「controller が DTO を組み立てられない要求は順1 より先に `INVALID_REQUEST` を返す」旨の例外を書き、本仕様はそれを参照する。**契約の変更**であり、#54（BFF）が消費する唯一の実体を動かす。
  - (c) **この組み合わせの応答を定義しないままにする** — 現状。ただし未定義のまま実装すると、実装者ごとに応答が割れる。
- **確定するまでしないこと**: **この組み合わせのテストを書かない**（AC-9-6 の但し書き・AC-12-9）。片方だけが該当する場合（ヘッダ不在のみ／構文不正のみ）のテストは書いてよい。
- **関係する AC**: AC-9-6（但し書き）・AC-9-7-a・AC-8-7・AC-12-9／契約 AC-1-4・AC-1-5・AC-9。

**Q-3. controller が呼ぶ interactor の抽象の置き場所**

- **一意に読み取れない点**: controller は **`usecase/interactor` を import しない**（AC-1-5 の許可リストに無い）ため、呼び出し先を**差し替え可能な抽象**として受け取る（AC-9-8-a）。その抽象をどこに置くかが決まっていない。`usecase/port` に入力ポートとして置く案は、**「ポートの数は4つ + 出力ポートに限り、追加は設計変更として扱う」**（AC-6-6）に触れる。
- **選択肢**:
  - (a) **`usecase/port` に入力ポート（interface）を置く** — 入出力を port に揃えられる。AC-6-6 の更新（ポートの数の定義変更）が要る。
  - (b) **`adapter/controller` が必要な最小の interface を自分で宣言する** — gateway が SQL 実行インターフェースを自分で宣言するのと同じ形（D-11）。port を増やさずに済むが、同種の宣言が controller 側に増える。
  - (c) **`driver/lambda` が関数値として渡す** — interface を作らない。結線は最も軽いが、複数の入力 DTO 型に対して型付けが弱くなる。
- **確定するまでしないこと**: **controller の実装で置き場所を確定させない。** ただし **AC-12-9 のテストは書ける**（置き場所ではなく差し替え可能性だけに依存するため。AC-12-9 に明記済み）。
- **関係する AC**: AC-9-8-a・AC-9-8-c・AC-1-4・AC-1-5・AC-6-6・AC-12-9。

**Q-4. gateway が受け取る SQL 実行インターフェースの具体的な形**

- **一意に読み取れない点**: D-11・AC-9-14 は「**gateway が宣言し、`driver/persistence` が pgx で実装し、`driver/lambda` が注入する**」ことと、SQL 文の置き場所（gateway 側）までを定めるが、**メソッドの粒度・行の走査の受け取り方・トランザクションの取得方法**は定めていない（AC-9-14-e）。ADR 0017 も `database/sql` 経由か pgx のネイティブ API かを実装 PR へ委ねている（AC-13-3）ため、そこからも一意に決まらない。
- **選択肢**:
  - (a) **`Query` / `Exec` / `Begin` の3メソッド**を宣言し、行の走査は `database/sql` の `Rows` 相当の最小 interface で gateway が受ける。汎用だが、走査の作法が gateway に露出する。
  - (b) **gateway が要る粒度で宣言する**（「1行取得」「複数行取得」「実行」）。走査は `driver` 側に閉じ、gateway は行の構造体スライスを受ける。gateway は素朴になるが、インターフェースが gateway の都合に依存して増える。
  - (c) **トランザクションを gateway が取得しない** — 単位を `driver/lambda` が決めて注入する。(a)(b) いずれとも組み合わせうる。原子性（AC-9-16-a・AC-10-7）をどちらの層が持つかが変わる。
- **確定するまでしないこと**: **gateway の実装とそのテストに入らない**（AC-9-14-e・AC-12-11・AC-13-17②）。**Fake の形がインターフェースの形に従属する**ため、確定前に書いたテストは確定後に必ず書き直しになる。
- **関係する AC**: D-11・AC-9-3・AC-9-14（特に 9-14-e）・AC-12-11・AC-13-3・AC-13-17／ADR 0017。
