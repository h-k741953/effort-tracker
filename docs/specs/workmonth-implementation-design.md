...
| 10-2 | DI 配線 | ここでのみ具体型を結線する（gateway・presenter・Clock・interactor）。**リクエストごとに presenter を生成**する（AC-7-6）。DB 接続はコールドスタート時に1度だけ確立し再利用する（予約同時実行数5。`docs/rules/cost-guardrails.md`）。**テスト可能性のための構造は AC-10-8・AC-10-9、テストは AC-12-15**（**2026-08-10 追記**）。**確立を行う呼び出し側は AC-10-15（エントリポイント）で、その取り出しは AC-10-18・テストは AC-12-17 ③**（**2026-08-12 追記**。**接続の本数と予約同時実行数の掛け算は未決＝Q-6**） |
...
**`docs/ai-collaboration.md` の責務分界表で人間の決定領域**（集約の切り方・ユビキタス言語の用語・技術選定・業務ルール）に触れる論点は、**2026-07-27・2026-07-28・2026-07-31・2026-08-05・2026-08-10 に人間が明示選択して確定した**（Issue #51）。確定内容は上表の D と各 AC に反映済みであり、下表は「何を選び、何を退けたか」を残すためのものである（Issue コメントに書き写さない。ADR 0004）。**2026-08-10 に起票した Q-5 も同日に確定し、決定14 として下表へ移した。2026-08-12 に、エントリポイント（D-12・AC-10-15〜AC-10-18）を起こした際に Q-6 を起票しており、これは未決のまま次節に置く**（`docs/rules/cost-guardrails.md` に触れるため**仕様工程が決めない**）。
...
>
> **HTTP 契約側の論点も確定済み**（`domain-api-http-contract.md`「人間が確定させた決定」。1〜3 は 2026-07-27、4 は 2026-07-28）。**本ファイル・HTTP 契約のいずれにも未決は無い**（決定9 は集約の復元＝実装設計に閉じ、HTTP の表現を変えない）。**これは 2026-07-31 時点の記述である。2026-08-05 に adapter 層の実装設計から Q-2〜Q-4 を起票したが、いずれも同日に確定して決定10〜決定12 へ移した**ため、**現在の未決は本ファイル・HTTP 契約のいずれにも無い**（次節）。**HTTP 契約側は起票から確定まで一貫して未決なし**であり、Q-2 は契約の変更を選択肢 (b) に含んでいたが**その案は退けられ、契約は変更していない**（決定10）。**2026-08-10 に `driver/persistence` の受け入れ条件を起こした際に Q-5 を起票したが、同日に人間が明示選択して確定し決定14 へ移した**ため、**現在の未決は本ファイル・HTTP 契約のいずれにも無い**（次節。**HTTP 契約側は起票から一貫して未決なし**であり、**決定14 も HTTP の表現を変えないため契約を変更していない**）。**その後、2026-08-12 にエントリポイントを起こした際 Q-6（`driver/persistence` が張る接続の本数と予約同時実行数の掛け算）を起票したため、現在の未決は本ファイルの Q-6 の1件である**（次節。**HTTP 契約側は引き続き未決なし**。**Q-6 も HTTP の表現を変えないため契約に触れない**）。
>
...
**現在の未決は Q-6 の1件である**（2026-08-12 起票）。
...
### Q-6. エントリポイントが張る接続の本数（**未決**。2026-08-12 起票）
