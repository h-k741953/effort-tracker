...
- **決定者**: 実装設計の各決定は仕様工程（AI）。ただし**集約の中身・ユビキタス言語への語の追加・技術選定・業務ルールに関わる決定は人間の決定事項**であり（`docs/ai-collaboration.md` 責務分界表）、該当する行の「決定者」欄には **人間** と記す。2026-07-27 と 2026-07-28 に上げた論点はすべて確定済みで、経緯と確定内容は末尾「人間が確定させた決定」に残す。**2026-07-30 に UC2（月次締め）の実装設計を追記した際、1件を人間へ上げて未決とした**（末尾「人間の決定を待っている論点」Q-1）
- **日付**: 2026-07-27（人間の決定事項を反映）／**2026-07-28 追記**（当該年月に属さない対象日への削除の確定を AC-2-5・AC-3-11・AC-4-2・AC-7-9 へ反映。業務ルールの実体は `daily-record-entry.md` D-9・AC-5-5）／**2026-07-30 追記**（UC2 の実装設計を AC-2-5・AC-4-3・AC-5-7〜AC-5-9・AC-7-10・AC-7-11・AC-13-12 へ追加）
...
| 2-5 | 永続化からの再構築 | `Reconstruct(...)` を公開し、`adapter/gateway` のみが使う。**状態遷移の検査は行わず**（保存済みの事実を復元する操作であり、遷移ではないため）、**値オブジェクトの妥当性と集約の不変条件**を検査する。不変条件は「対象日で一意」（AC-2-6）と「**すべての対象日が当該年月に属する**」（AC-2-1 の「当該年月の稼働実績だけを保持する」）の2つで、いずれの違反も `ErrInvalidValue` で失敗させる（AC-3-11。正常な経路では作られない行に対する防御であり、利用者の要求の不正ではないため `ErrDateOutOfMonth` を使わない）。**確定済みの超過／不足も引数として受け取る**（AC-5-9。`Draft` は「値なし」、締め後は値あり。永続化側の表現は AC-10-5）。**受け取った超過／不足と状態の整合を検査するか否かは未決**（Q-1。2026-07-30 に人間へ上げた）であり、確定するまで3つ目の不変条件を足さない |
...
| 5-9 | 永続化との往復 | 確定済みの超過／不足は `Reconstruct` の**末尾の引数**として `excess`・`shortfall` の順（いずれも `*WorkingHours`）で受け取る（AC-2-5）。**未確定は `nil`** とし、永続化側の NULL と1対1に対応させる（AC-10-5）。**復元時に再計算しない**（締めた時点の事実を固定する。UC2 D-2・AC-3-2）。ポインタで受け取っても集約は値として保持し、呼び出し側から書き換えられないようにする（AC-2-3） |
...
| 12-7 | Fake が集約を複製する経路 | `WorkMonthRepository` の Fake は `Reconstruct` で複製を返す（保存済みの状態が `Save` を経ずに変わらないことを実装と同じ性質で再現するため）。**確定済みの超過／不足も複製へ引き継ぐ**（AC-5-9）。引き継がないと締め済・承認済の勤務月が未確定として復元され、テストが実装より弱くなる |
...
**`docs/ai-collaboration.md` の責務分界表で人間の決定領域**（集約の切り方・ユビキタス言語の用語・技術選定・業務ルール）に触れる論点は、**2026-07-27 と 2026-07-28 に人間が明示選択して確定した**（Issue #51）。確定内容は上表の D と各 AC に反映済みであり、下表は「何を選び、何を退けたか」を残すためのものである（Issue コメントに書き写さない。ADR 0004）。**2026-07-30 に UC2 の実装設計から新たに1件（Q-1）を上げており、それだけが未決である**（次節）。
...
> **HTTP 契約側の論点も確定済み**（`domain-api-http-contract.md`「人間が確定させた決定」。1〜3 は 2026-07-27、4 は 2026-07-28）。**未決は本ファイルの Q-1 のみ**で、HTTP 契約側に未決は無い（Q-1 は集約の復元＝実装設計に閉じ、HTTP の表現を変えない）。
>
...
**推測で埋めない**（`docs/ai-collaboration.md`「AIの停止条件」）。下記は 2026-07-30 に UC2（月次締め）の実装設計を起こす過程で見つかり、**仕様工程では決めずに人間へ上げた**論点である。確定したら本節から上表「人間が確定させた決定」へ移し、該当 AC を更新する。

| # | 論点 | なぜ人間の決定か | 選択肢 | 影響範囲 |
|---|---|---|---|---|
| Q-1 | **`Reconstruct` が受け取る「確定済みの超過／不足」と状態の整合を、集約の不変条件として検査するか**（AC-2-5・AC-5-9）。`Draft` なのに値がある行／締め済・承認済なのに値が無い行を、復元時に弾くか通すか | AC-2-5 は集約の不変条件を**2つ**と明示的に数え上げており、3つ目を足すことは**集約が守る範囲を広げる**変更にあたる（`docs/ai-collaboration.md`「ドメイン境界・集約の切り方」は人間の決定領域）。決定8（当該年月外の対象日の復元）と同じ場所・同じ種類の判断である | **(a) 検査する。** 3つ目の不変条件として扱い、違反は `ErrInvalidValue`（決定8 と同じ思想＝壊れた行を黙って通さない）／**(b) 検査しない。** 受け取った値をそのまま復元し、整合は永続化側（AC-10-5 の NULL 可否）と gateway に委ねる。AC-2-5 の「不変条件は2つ」を維持する | AC-2-5・AC-5-9。**HTTP の表現は変わらない**（正常な経路では作られない行のみが対象）。tester は確定するまでこの組み合わせのテストを書かない |

> **退けるべき案として記録する**: 「`Reconstruct` は超過／不足を引数に取らず、締め済・承認済なら復元時に総稼働時間と精算幅から再計算する」。締めた時点の事実を固定するという業務決定（`monthly-closing.md` D-2・AC-3-2）を壊し、精算幅の変更が過去の勤務月へ遡及しうるため、**選択肢に含めない**。
>
> **Q-1 が未決でも UC2 のテスト・実装は進められる。** `Reconstruct` の**引数**（確定済みの超過／不足を受け取ること）は D-2 と AC-10-5 の帰結として確定しており、未決なのは**検査の有無**だけである。
