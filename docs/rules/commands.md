...
|---|---|
| `make help` | ターゲット一覧 |
| `make verify` | `lint` + `test` + `check-domain-deps` + `check-skills` + `check-go-module-pins` + `scan-secrets`（コミット前・PR前） |
| `make test` | 全レイヤーのテスト |
...
>
> **Go の依存を追加・更新するときは `services/api/go.mod` を先に直し、`.devcontainer/Dockerfile` の `ARG` pin を追随させ、devcontainer をリビルドする。実行時のモジュール取得は成立しない**（`docs/specs/devcontainer-go-module-policy.md`）。
