# AC-9-c（docs/specs/cognito-auth-infra.md）の検査。
#
# 本ファイルが検査するのは、User Pool が SMS の送信設定を持たないこと（当該属性が空・未設定であること。AC-3-2） だけである。
#
...
run "sms_configuration_is_absent" {
  command = plan

  assert {
    condition     = length(aws_cognito_user_pool.this.sms_configuration) == 0
    error_message = "User Pool は SMS の送信設定（sms_configuration）を持ってはならない（AC-3-2）"
  }
}
