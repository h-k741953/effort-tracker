...
このコマンドに渡された引数全体は `$ARGUMENTS` に入っている。**`$ARGUMENTS` の
値をシェル文字列へ連結・埋め込みしてはならない。** bash 前処理
...
代わりに `.claude/scripts/issue-gate.sh` を **`arg` モードで、`$ARGUMENTS` の
値を Bash ツールの argv 上の独立した1要素として**呼び出す。シェルの引用符
（`"..."`）で括ってコマンド文字列に連結するのではなく、ツール呼び出しの
引数列に直接値として渡すこと。
