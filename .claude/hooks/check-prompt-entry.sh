...
#   緩めないこと。
#
#   T-1（て + 区切り）の「区切り」は、終助詞（ね/よ/な。0個以上の繰り返しを
#   許す）を挟んでもなお成立する（B-1-c）。区切りの列挙自体は、往復1（実測）
#   で終助詞・閉じ記号・\r を欠いていたために `実装してね` `実装して」`
#   `実装して\r\nよろしく` が素通りしていたことを踏まえて拡張したものである。
#   **列挙で塞げるのは実測で見つかった穴の範囲であり、「後置き文字列では
#   もう逃げられない」ことを保証するものではない**（列挙外の後置き文字は
#   今後も残りうる。docs/specs/orchestrator-entry-hook.md AC-6-9）。
#
#   T-2（て + 依頼接尾）も、往復2（実測）で T-1 と同じ二段構え（依頼接尾(1+)
#   + 依頼活用尾(0..1) + 終助詞*(0+) + 区切り）へ改めた。当初の T-2 は
#   「て + 依頼接尾」で閉じており、接尾の直後が何であっても一致していたため、
#   `実装してくれた`（過去形の報告）`対応してもらえなかった`（否定過去）
#   `実装してほしいと言われた`（引用・伝聞）等の最頻の言い回しが偽陽性で
#   ブロックされていた。境界規則を入れた結果、T-2 は T-1 と同様に「区切りの
#   列挙で閉じる」形になった。**したがって「T-2 は後続を問わず拾うため
#   後置きだけでは逃げにくい」という主張はもう成り立たない。** T-2 側も
#   T-1 と同じ後置きすり抜け（列挙外の文字を後ろに置けば一致が外れる）を
#   引き受けている（docs/specs/orchestrator-entry-hook.md B-1-c）。
#
...
# ==============================================================================
# 先頭の空白文字（半角スペース／タブ／改行 \n／復帰 \r）を除いた直後が、
# 半角8文字の "[direct]" に完全一致するかを見る。sed -z でファイル全体を
# 1つのバッファとして扱い、行頭アンカーではなく「ファイル冒頭からの空白」を
# 一括で取り除く（複数行にまたがる先頭空白・改行を無視するため。AC-1-2）。
#
# \r をこの4文字に含めるのは意図的な決定（往復2 W-3）。AC-2 側（B-1-c の SEP）
# は CRLF 由来の \r を区切りとしてブロック側に足しているため（AC-8-7）、
# 同じ CRLF 入力に対して唯一の逃げ道である [direct] だけ \r を除去しないのは
# 非対称になる（\r\n[direct] 実装して がブロックされたうえに回避手段も
# 塞がれる）。全角スペースは含めない（AC-1 は曖昧な一致を許さない側であり、
# 半角空白 [ \t\n\r] のみを対象とする。全角スペースは通常の文字として扱われ、
# [direct] トークンの前に置かれれば「冒頭でない」としてブロックされる）。
TRIMMED_FILE="${WORK}/trimmed.txt"
sed -z 's/^[ \t\n\r]*//' "$PROMPT_FILE" > "$TRIMMED_FILE"
...
# --- T: 指示語尾（B-1-c） ------------------------------------------------------
# 区切り（区切り記号 + 空白 + 閉じ記号 + \r + 文字列終端）。終端は ERE の `$` を
# 使う。`$` は既定（-z なし）の grep が行単位で処理するときの「行末（＝直後の
# 改行の手前）」に一致するので、jq -r が付与する末尾の改行と組み合わせて
# 「文字列の終端 / 改行（\n）」の両方をこの1つのアンカーで表せる（多バイトの
# 改行バイト自体をパターンへ書く必要がない）。
#
# 閉じ記号（」』）)”":：;；）は往復1 C-1（実測）で塞いだ区切りの追加分。
# \r（復帰。CRLF 由来）は往復1 W-3（実測）で塞いだ区切りの追加分で、grep が
# 行区切りとして消費するのは \n のみのため、\r は行内容として残り明示的な
# 区切り扱いが要る（docs/specs/orchestrator-entry-hook.md AC-8-7）。\r は
# 1バイト ASCII の制御文字であり、シェルの ANSI-C クォート $'\r' で生成した
# うえで文字列連結として SEP へ埋め込む（character class には入れない。
# 多バイト文字ではないため事故要因にはならないが、方針を統一するため）。
CR=$'\r'
SEP='( |	|　|'"${CR}"'|。|、|．|，|\.|,|!|！|\?|？|」|』|）|\)|”|"|:|：|;|；)'
# 終助詞（ね/よ/な）。**区切りそのものではなく「区切りの前に置ける文字」として
# 定義する**のが要点（B-1-c）。0個以上の繰り返しを許すので `実装してよね` も
# 一致する。終助詞を区切り集合へ直接入れると `修正してよかった`
# `まだ実装してないよね` `対応してねぇの?`（終助詞の直後がなお本文）まで
# 誤爆する。二段構え（終助詞* → 区切り）にすることで、この3例は区切り判定に
# 届かず通過する（AC-3-23〜3-25）。
SFP='(ね|よ|な)*'
# 依頼接尾（T-2）。1個以上の連接を許し、連接するときに限り2つの接尾の間に
# 「て」を1つ挟んでよい（`しておいてください` / `してもらってください`）。
# REQCHAIN = REQ ( て? REQ )* という形にすることで、連接を許しつつ、
# **末尾の接尾の直後に「て」を置くことは許さない**（グループの外に単独の
# 「て」を置く経路が無いため）。許すと `実装してくれて、ありがとう` が
# 区切り「、」に一致し、感謝・報告の最頻表現が偽陽性になる（往復2 W-1）。
REQ='(くれ|ください|下さい|ほしい|欲しい|おいて|もらえ|もらい|もらっ|いただけ|いただき)'
# 「て」を単独の group `(て)?` として囲むこと。`て?` のように多バイト文字へ
# 直接 `?` を付けると、LC_ALL=C 下では `?` がマルチバイト文字の最終バイト
# 1つだけに適用され、意図した「0個または1個」にならない（character class の
# 事故（Makefile 記載の mawk 型）と同型の、quantifier 版の事故。実測で
# `(て?REQ)*` が `おいてください` に一致しないことを確認して発見した）。
REQCHAIN="${REQ}((て)?${REQ})*"
# 依頼活用尾（T-2）。依頼接尾の直後に、依頼として自然な活用形を0個または1個
# だけ許す（繰り返さない）。`た` `ました` のような過去形はここに無いため、
# 過去形の報告（`〜てくれた` `〜てもらいました`）は拾われない
# （往復2 W-1、docs/specs/orchestrator-entry-hook.md B-1-c「T-2 に境界規則を
# 置く理由」）。「除外条件」としてではなく「拾う側の列挙」として書く
# （同節の方針）。
REQATT='(ていい|てもいい|ますか|ませんか|ないか|たい|ます|ない|る)'
T1='て'"${SFP}"'('"${SEP}"'|$)'
# T2 = て + 依頼接尾(1個以上、連接可) + 依頼活用尾(0個または1個) +
#      終助詞*(0個以上) + 区切り。T-1 と同じ二段構え（終助詞* → 区切り）を
#      踏襲し、接尾・活用尾の列挙に無い後続文字（過去形・引用・連体修飾等）は
#      拾わない（AC-3-26〜3-33）。
T2='て'"${REQCHAIN}${REQATT}"'?'"${SFP}"'('"${SEP}"'|$)'
T3='といて'
